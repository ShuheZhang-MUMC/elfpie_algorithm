function [lambda,n_LED,CTF_object0,CTF_object,NA,pix_CCD,...
            plane_wave_org,...
            plane_wave_new,df,sample_size] = init_enviroment(d,if_show)

%{
------------input parameters:----------
    d:        maximum LED position shift

    if_show = 1：show LED position shift

------------output parameters:----------

    lambda:          wave length (um)

    n_LED:           number of LEDs in x-direction, odd number, the total number of
                     LEDs is n_LED^2

    CTF_object0:     CTF in ideal condition (no aberration)
    CTF_object:      CTF (maybe) with no aberration

    NA:              NA value of the image system

    pix_CCD:         Pixel size of the low-res images

    Mag:             system magnification

    plane_wave_org:  Plane wave direction without shift
    plane_wave_new:  Plane wave direction with random shift

    df:              Frequency sample size 
